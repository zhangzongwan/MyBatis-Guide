...
本仓库旨在帮助所有新手程序员（包括作者自己）理解并掌握Mybatis框架。

mybatis用户指南基于[Mybatis中文网](https://mybatis.net.cn/)和作者对Mybatis的理解所编写，若有错误，请积极批评指正。

## 目录

注意：标题中带有*号的不做强制学习的要求。
...
上述的代码包括其他常见的依赖能够在[Maven仓库](https://mvnrepository.com/)中找到



### 2.构建 SqlSessionFactory

#### 从 XML 中构建 SqlSessionFactory（建议使用）

每个基于 MyBatis 的应用都是以一个 **SqlSessionFactory** 的实例为核心的。SqlSessionFactory 的实例可以通过 SqlSessionFactoryBuilder 获得。而 SqlSessionFactoryBuilder 则可以从 XML 配置文件或一个预先配置的 Configuration 实例来构建出 SqlSessionFactory 实例。

![SqlSessionFactory](Mybatis用户指南.assets/SqlSessionFactory.svg)

从 XML 文件中构建 SqlSessionFactory 的实例非常简单，建议使用**类路径下的资源文件**进行配置。 
...
XML 配置文件中包含了对 MyBatis 系统的核心设置，包括**获取数据库连接实例的数据源（DataSource）**以及**决定事务作用域和控制方式的事务管理器（TransactionManager）**。后面会再探讨 XML 配置文件的详细内容，这里先给出一个简单的示例：
...
mappers 元素则包含了一组映射器（mapper），这些映射器的 XML 映射文件包含了 SQL 代码和映射定义信息。（Mybatis通过mappers 元素找到对应的mapper，所以**在写xml文件时要记得添加映射文件信息**）
...
try (SqlSession session = sqlSessionFactory.openSession()) {
  Blog blog = (Blog) session.selectOne("org.mybatis.example.BlogMapper.selectBlog", 101);//不建议使用
}
```
...
try (SqlSession session = sqlSessionFactory.openSession()) {
  ...
  Blog blog = mapper.selectBlog(101);//建议使用
}
```

### 探究已映射的 SQL 语句

SqlSession 和 Mapper通过映射SQL语句执行数据库操作。Mybatis有两种使用SQL语句的方式，可以通过**XML定义**，也可以通过**注解**定义。

这里给出一个XML 映射语句的简单示例，它应该可以满足上个示例中 SqlSession 的调用。
...
SqlSessionFactory 一旦被创建就**应该在应用的运行期间一直存在**，没有任何理由丢弃它或重新创建另一个实例。 使用 SqlSessionFactory 的最佳实践是在应用运行期间不要重复创建多次，多次重建 SqlSessionFactory 被视为一种代码“坏习惯”。因此 SqlSessionFactory 的最佳作用域是应用作用域。 有很多方法可以做到，最简单的就是使用单例模式或者静态单例模式。
...
**简单来说，类型处理器就是将数据库中的数据类型转换为Java中的数据类型的工具。**
...
- 在类型处理器的配置元素（typeHandler 元素）上增加一个 `javaType` 属性（比如：`javaType="String"`）；**（也就是说，通过javaType可以在不需要改变类型处理器的情况下，改变型处理器处理的 Java 类型）**
...
> **简单来说，#和\$的区别在于#相当于对数据加上单引号，$当于直接显示数据。**
>
> 举个例子，
>
> #对传入的参数视为字符串，也就是它会预编译，select * from user where name = #{name}，比如我传一个aaa，那么传过来就是 select * from user where name = 'aaa'；
>
> \$不会将传入的值进行预编译，select * from user where name=${name}，比如我传一个aaa，那么传过来就是 select * from user where name = aaa；
>
> **加不加引号的区别在于是否会导致sql注入**
>
> 比如：
>
> 用户进行一个登录操作，后台sql验证是这样的：
>
> select * from user where username=#{name} and password = #{pwd}
>
> 如果前台传来的用户名是“wang”，密码是 “1 or 1=1”，
>
> 用#的方式就不会出现sql注入，
>
> 而如果换成$方式，sql语句就变成了 
...
简单来说，这个事例就是先查作者，通过查询到的作者信息再查询博客。
...
**其它所有的属性将会被自动加载，只要它们的列名和属性名相匹配。**（这就是blogResult里面没有把BLog的映射关系写出来的原因）
...
之前，你已经看到了一个非常复杂的嵌套关联的例子。 下面的例子则是一个非常简单的例子，用于演示嵌套结果映射如何工作。 现在我们**将博客表和作者表连接在一起，而不是执行一个独立的查询语句**，就像这样：
...
if标签会通过test属性的表达式进行判断，若为真，则执行标签中的内容。  
...
有时候，我们不想使用所有的条件，而**只是想从多个条件中选择一个使用**。针对这种情况，MyBatis 提供了 choose 元素，它有点像 Java 中的 switch 语句。
